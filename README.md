...
- `EMBEDDINGS_BACKEND`: The backend to use for the embeddings. Supported values are `ollama`, or `infinity`. Default is `infinity`.
...
- `LLM_BACKEND`: The backend to use for the LLM calls. Supported values are `openai`, `bedrock` or `ollama`. Default is `openai`.
- `LLM_BACKEND_URL`: The URL of the LLM backend, relevant only for `ollama` and `openai` backends. Default is `http://localhost:11434`.
- `TESTS_BACKEND`: The backend to use for the tests. Supported values are `openai`, `bedrock`, or `ollama`. Default is `openai`.
- `TESTS_BACKEND_URL`: The URL of the tests backend, relevant only for `ollama` and `openai` backends. Default is `http://localhost:11434`.
