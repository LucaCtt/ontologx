...
class GraphBuilderOutputState:
    ...
    output_graph: GraphDocument | None = None
...
def _initialize_state(state: _GraphBuilderState) -> _GraphBuilderState:
    ...
    messages: list[AnyMessage] = state.messages if state.messages else [SystemMessage(state.prompt_build_graph)]
    ...
    structured_llm: BaseChatModel = state["llm"].with_structured_output(  # type: ignore[attr-defined]
        build_dynamic_model(state.ontology),
        include_raw=True,
        method="function_calling",
    )

    return _GraphBuilderState(**state.__dict__, llm=structured_llm, messages=messages)


def _build_graph(state: _GraphBuilderState) -> _GraphBuilderState:
...
def _cleanup_messages(state: _GraphBuilderState) -> _GraphBuilderState:
    ...
    if n_conversations >= state.max_conversation_history:
        ...
        for i in reversed(range(len(messages_to_keep))):
            if isinstance(messages_to_keep[i], HumanMessage) and str(messages_to_keep[i].content).startswith("Event: "):
                ...
                if current_conversations == state.max_conversation_history:
...
def _get_next_node(state: _GraphBuilderState) -> str:
    """Determine the next node based on the current state."""
    if state.output_graph is None and state.n_corrections_done < state.max_correction_steps:
