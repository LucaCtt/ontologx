"""Pydantic models for message graphs generated by the LLM."""
...
class BaseKnowledgeGraph(BaseModel):
    """Base graph model to represent the knowledge graph generated by a LLM from a message.
    ...
    def graph(self, source_message: str, context: dict) -> GraphDocument:
        """Convert the generated message graph to a GraphDocument.
        ...
        Args:
            source_message (str): The message that was parsed to create this graph.
            context (dict): The context of the message, which will be used as metadata in the GraphDocument.

        Returns:
            GraphDocument: A GraphDocument containing the nodes and relationships of the message graph.
        ...
        return GraphDocument(
            ...
            source=Document(page_content=source_message, metadata={"context": context}),
        )
...
def build_dynamic_model(ontology: GraphDocument) -> type[BaseKnowledgeGraph]:
    """Build a dynamic message graph model based on the ontology."""
    ...
    class Property(BaseModel):
        """A property of a node in the message knowledge graph."""
    ...
    class Node(BaseModel):
        """A node in the message knowledge graph."""
    ...
    class Relationship(BaseModel):
        """A relationship between two nodes in the message knowledge graph."""
        ...
        __doc__ = (
            "A relationship between two nodes in the message graph. "
    ...
    class KnowledgeGraph(BaseKnowledgeGraph):
        """An message knowledge graph composed of nodes and relationships."""
        ...
        def validate_relationships(self) -> "KnowledgeGraph":
        ...
        # TODO: maybe re-enable this validation in the future
        #@model_validator(mode="after")
        #def validate_message(self) -> "MessageGraph":
        #    """Validate the message graph.
        #
...
def build_baseline_prompt(ontology: GraphDocument, base_prompt: str) -> str:
    """Build a baseline prompt for the LLM to generate a message graph."""
    valid = _OntologyValidValues(ontology)
